...
def pack_a_like_official_vecout_tscm_sample(a):
    # Official matmul_tscm_src_vecout/gen_data.py stores A as
    # [K/16, M/16, 16, 16] before the kernel copies VECOUT -> TSCM.
    return a.reshape(2, 16, 2, 16).permute(2, 0, 1, 3).contiguous().reshape(32, 32)
...
def main():
    ...
    a = torch.randn((32, 32), device="npu", dtype=torch.float16).contiguous()
    b = torch.randn((32, 256), device="npu", dtype=torch.float16).contiguous()
    bias = torch.randn((256,), device="npu", dtype=torch.float32).contiguous()
    a_tscm = pack_a_like_official_vecout_tscm_sample(a)

    out = torch.ops.npu.matmul_tscm_vecout_probe(a_tscm, b, bias)
    ...
    if args.benchmark:
        ...
        result["candidate_us"] = time_us(
            lambda: torch.ops.npu.matmul_tscm_vecout_probe(a_tscm, b, bias), args.warmup, args.repeat
        )
