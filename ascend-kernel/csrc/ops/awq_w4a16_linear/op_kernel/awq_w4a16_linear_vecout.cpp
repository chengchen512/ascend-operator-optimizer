...
namespace AwqW4A16LinearVecout {
...
constexpr uint32_t BasicK = 256;
constexpr uint32_t TileN = 128;
constexpr uint32_t VectorCols = 128;
constexpr uint32_t Int4PerPack = 8;
...
static_assert(VectorCols == TileN, "This kernel assumes one vector op covers the full N tile.");
...
public:
    ...
    __aicore__ inline void Process(AscendC::TPipe *pipe)
    {
        ...
        const uint32_t tileCountN = n_ / TileN;
        if (blockIdx >= tileCountN) {
            return;
        }
        ...
        for (uint32_t kg = 0; kg < matmulGroupNum; ++kg) {
            ...
            Add(bVec, bVec, offsetLocal0, VectorCols, static_cast<uint8_t>(GroupK), aqParams);
            PipeBarrier<PIPE_V>();
            Mul(bVec, bVec, scaleLocal0, VectorCols, static_cast<uint8_t>(GroupK), aqParams);
            ...
            Add(bVec[secondGroupOffset], bVec[secondGroupOffset], offsetLocal1, VectorCols,
                static_cast<uint8_t>(GroupK), aqParams);
            PipeBarrier<PIPE_V>();
            Mul(bVec[secondGroupOffset], bVec[secondGroupOffset], scaleLocal1, VectorCols,
                static_cast<uint8_t>(GroupK), aqParams);
            PipeBarrier<PIPE_V>();
...
extern "C" __global__ __aicore__ void awq_w4a16_linear_vecout(
    ...
    uint32_t colOffset, GM_ADDR workspace, GM_ADDR tilingGm)
{
    ...
    AwqW4A16LinearVecout::Kernel kernel;
