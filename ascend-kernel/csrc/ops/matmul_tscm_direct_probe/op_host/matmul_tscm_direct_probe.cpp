...
#include "aclrtlaunch_matmul_tscm_direct_probe.h"
...
int64_t GetWorkspaceBytes()
{
    ...
    TORCH_CHECK(platform != nullptr, "matmul_tscm_direct_probe: failed to initialize AscendC platform");
...
TilingCache MakeTilingCache()
{
    ...
    tiling.SetDim(platform->GetCoreNumAic());
    ...
    TORCH_CHECK(tiling.GetTiling(cube_tiling) != -1, "matmul_tscm_direct_probe: GetTiling failed");
    TORCH_CHECK(cube_tiling.usedCoreNum > 0, "matmul_tscm_direct_probe: expected positive usedCoreNum");
...
at::Tensor matmul_tscm_direct_probe(const at::Tensor &a, const at::Tensor &b, const at::Tensor &bias)
{
    TORCH_CHECK(a.dim() == 2 && b.dim() == 2 && bias.dim() == 1,
                "matmul_tscm_direct_probe: expected a=[64,64], b=[64,64], bias=[64]");
    TORCH_CHECK(a.scalar_type() == at::kHalf && b.scalar_type() == at::kHalf,
                "matmul_tscm_direct_probe: a/b must be float16");
    TORCH_CHECK(bias.scalar_type() == at::kFloat, "matmul_tscm_direct_probe: bias must be float32");
    TORCH_CHECK(a.is_contiguous() && b.is_contiguous() && bias.is_contiguous(),
                "matmul_tscm_direct_probe: all inputs must be contiguous");
    TORCH_CHECK(a.size(0) == kM && a.size(1) == kK && b.size(0) == kK && b.size(1) == kN &&
                    bias.numel() == kN,
                "matmul_tscm_direct_probe: only supports M=N=K=64");
    ...
    EXEC_KERNEL_CMD(matmul_tscm_direct_probe, block_dim, a, b, bias, c, workspace, tiling_dev);
    return c;
}
