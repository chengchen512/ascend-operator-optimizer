# asc\_threadfence<a name="ZH-CN_TOPIC_0000002484776312"></a>
...
在SIMT编程范式中，来自不同线程对同一份内存的读写操作可能造成数据竞争（Data Race）。asc\_threadfence接口用于保证不同线程对同一份全局、共享内存的访问过程中，写入操作的时序性。它不会阻塞线程，仅保证内存操作的可见性顺序。
