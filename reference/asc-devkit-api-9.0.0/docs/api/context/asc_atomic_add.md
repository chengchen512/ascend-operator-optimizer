# asc\_atomic\_add<a name="ZH-CN_TOPIC_0000002516816293"></a>
...
-   Unified Buffer数据的原子加操作
    ...
    ```
    __simt_callee__ inline half asc_atomic_add(__ubuf__ half *address, half val)
    ```

    ```
    __simt_callee__ inline bfloat16_t asc_atomic_add(__ubuf__ bfloat16_t *address, bfloat16_t val)
    ```

    ```
    __simt_callee__ inline half2 asc_atomic_add(__ubuf__ half2 *address, half2 val)
    ```

    ```
    __simt_callee__ inline bfloat16x2_t asc_atomic_add(__ubuf__ bfloat16x2_t *address, bfloat16x2_t val)
    ```

-   Global Memory数据的原子加操作
    ...
    ```
    __simt_callee__ inline half asc_atomic_add(__gm__ half *address, half val)
    ```

    ```
    __simt_callee__ inline bfloat16_t asc_atomic_add(__gm__ bfloat16_t *address, bfloat16_t val)
    ```

    ```
    __simt_callee__ inline half2 asc_atomic_add(__gm__ half2 *address, half2 val)
    ```

    ```
    __simt_callee__ inline bfloat16x2_t asc_atomic_add(__gm__ bfloat16x2_t *address, bfloat16x2_t val)
    ```
...
<tbody><tr id="row42461942101815"><td class="cellrowborder" valign="top" width="14.12%" headers="mcps1.2.4.1.1 "><p id="p284425844311"><a name="p284425844311"></a><a name="p284425844311"></a>address</p>
</td>
<td class="cellrowborder" valign="top" width="14.469999999999999%" headers="mcps1.2.4.1.2 "><p id="p158449584436"><a name="p158449584436"></a><a name="p158449584436"></a>输出</p>
</td>
<td class="cellrowborder" valign="top" width="71.41%" headers="mcps1.2.4.1.3 "><p id="p6389114616514"><a name="p6389114616514"></a><a name="p6389114616514"></a><span id="ph38777222303"><a name="ph38777222303"></a><a name="ph38777222303"></a>Unified Buffer</span>或<span id="ph128774221307"><a name="ph128774221307"></a><a name="ph128774221307"></a>Global Memory</span>的地址。</p>
</td>
</tr>
<tr id="row625424812813"><td class="cellrowborder" valign="top" width="14.12%" headers="mcps1.2.4.1.1 "><p id="p6255648487"><a name="p6255648487"></a><a name="p6255648487"></a>val</p>
</td>
