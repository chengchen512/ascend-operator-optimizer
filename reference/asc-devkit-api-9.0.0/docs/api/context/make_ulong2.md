# make\_ulong2<a name="ZH-CN_TOPIC_0000002546527537"></a>
...
```
__simt_callee__ inline ulong2 make_ulong2(unsigned long int x, unsigned long int y)
```
...
```
__simt_vf__ __launch_bounds__(1024) inline void kernel_make_ulong2(__gm__ ulong2* dst, __gm__ unsigned long int* x, __gm__ unsigned long int* y)
{
    int idx = threadIdx.x + blockIdx.x * blockDim.x;
    dst[idx] = make_ulong2(x[idx], y[idx]);
}
```
