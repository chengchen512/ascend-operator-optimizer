# hsin<a name="ZH-CN_TOPIC_0000002533360245"></a>
...
```
__simt_callee__ inline half hsin(half x)
```

```
__simt_callee__ inline bfloat16_t hsin(bfloat16_t x)
```
...
-   调用示例一：
    ...
    ```
    bfloat16_t x = 0;
    bfloat16_t result = hsin(x); // 返回值为0
    ```
