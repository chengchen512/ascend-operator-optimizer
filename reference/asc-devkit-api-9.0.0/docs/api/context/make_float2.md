# make\_float2<a name="ZH-CN_TOPIC_0000002514967604"></a>
...
使用给定的两个float类型的数据创建一个float2类型的向量。

## 函数原型<a name="section620mcpsimp"></a>

```
__simt_callee__ inline float2 make_float2(float x, float y)
```
...
```
__simt_vf__ __launch_bounds__(1024) inline void kernel_make_float2(__gm__ float2* dst, __gm__ float* x, __gm__ float* y)
{
    int idx = threadIdx.x + blockIdx.x * blockDim.x;
    dst[idx] = make_float2(x[idx], y[idx]);
}
```
