# make\_ulonglong2<a name="ZH-CN_TOPIC_0000002514967610"></a>
...
使用给定的两个unsigned long long int类型的数据创建一个ulonglong2类型的向量。

## 函数原型<a name="section620mcpsimp"></a>

```
__simt_callee__ inline ulonglong2 make_ulonglong2(unsigned long long int x, unsigned long long int y)
```
...
由两个unsigned long long int类型数字构成的新ulonglong2向量。
...
```
__simt_vf__ __launch_bounds__(1024) inline void kernel_make_ulonglong2(__gm__ ulonglong2* dst, , __gm__ unsigned long long int* x, __gm__ unsigned long long int* y)
{
    int idx = threadIdx.x + blockIdx.x * blockDim.x;
    dst[idx] = make_ulonglong2(x[idx], y[idx]);
}
```
