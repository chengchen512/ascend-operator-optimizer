# make\_longlong2<a name="ZH-CN_TOPIC_0000002546487549"></a>
...
使用给定的两个long long int类型的数据创建一个longlong2类型的向量。

## 函数原型<a name="section620mcpsimp"></a>

```
__simt_callee__ inline longlong2 make_longlong2(long long int x, long long int y)
```
...
```
__simt_vf__ __launch_bounds__(1024) inline void kernel_make_longlong2(__gm__ longlong2* dst, __gm__ long long int* x, __gm__ long long int* y)
{
    int idx = threadIdx.x + blockIdx.x * blockDim.x;
    dst[idx] = make_longlong2(x[idx], y[idx]);
}
```
