# \_\_ull2half\_rn<a name="ZH-CN_TOPIC_0000002533069841"></a>
...
将uint64类型数据转换为half类型数据，并四舍五入到最接近的偶数。

## 函数原型<a name="section620mcpsimp"></a>

```
__simt_callee__ inline half __ull2half_rn(const unsigned long long int x)
```
...
输入的uint64数据转换成的half数据，并四舍五入到最接近的偶数。
