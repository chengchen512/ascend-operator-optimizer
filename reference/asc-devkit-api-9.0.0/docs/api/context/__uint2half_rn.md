# \_\_uint2half\_rn<a name="ZH-CN_TOPIC_0000002501429820"></a>
...
将uint32类型数据转换为half类型数据，并四舍五入到最接近的偶数。

## 函数原型<a name="section620mcpsimp"></a>

```
_simt_callee__ inline half __uint2half_rn(const unsigned int x)
```
...
输入的uint32数据转换成的half数据，并四舍五入到最接近的偶数。
