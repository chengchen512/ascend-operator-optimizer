...
内容来自仓库内 `.agents/skills/` 下的本地 skill，不包含远端服务器上的临时实验文件。
...
- `skill_usage.md`: 本项目中 skill 的实际使用记录和关键测试结果。
- `AWQ_ASCENDC_OPT_EXPERIENCE.md`: 本次 AWQ W4A16 AscendC 算子优化经验总结。
...
- 需要继续性能优化时，优先查 `reference/skills/ascendc-operator-performance-optim/references/`。
  - 远端 910B2C / `Ascend910_9382` 资源边界、`GetTiling failed`、`507015`、
    VECOUT/TSCM/queue/workspace 约束优先查
